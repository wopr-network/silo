...
export function resolveCorsOrigin(opts: { host: string; corsEnv: string | undefined }): CorsOriginResult {
  ...
  if (corsValue) {
    ...
    for (const entry of entries) {
      if (!/^https?:\/\/[^/]+$/.test(entry)) {
        throw new Error(
          `SILO_CORS_ORIGIN must be bare origins like https://app.example.com (comma-separated for multiple), not ${entry}. ` +
  ...
  if (!isLoopback) {
    throw new Error(
      `SILO_CORS_ORIGIN must be set when binding to non-loopback address "${opts.host}". ` +
        "Without an explicit CORS origin, any website on the network can make cross-origin requests to this server. " +
        'Set SILO_CORS_ORIGIN to the allowed origin (e.g. "https://my-app.example.com") or use a loopback address. ' +
